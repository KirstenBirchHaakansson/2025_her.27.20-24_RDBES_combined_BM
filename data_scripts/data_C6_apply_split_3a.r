...
split <-
  read.table(
    paste0(
      path_data,
      "updated_split_2000_2024_SD20SD21.csv"
    ),
    sep = ";",
    header = T
  )
...
canum <-
  read.table(paste0(
    path_data,
    "updated_canum_with_imputations_2000_2024.csv"),
    sep = ";", header = T
  )
...
write.table(
  splitted,
  paste(path_data, "updated_canum_wbss_nsas_2000-2024.csv", sep = ""),
  sep = ",",
  row.names = F
)
saveRDS(splitted,
        paste(path_data, "updated_canum_wbss_nsas_2000-2024.rsd", sep = ""))
