...
dat_sum <- summarise(group_by(dat_old, year, area, quarter, fleet, wr),
                     weca_g = sum(weca * canum, na.rm = T) /
                        sum(canum, na.rm = T),
                     canum_1000 = sum(canum * 1000, na.rm = T))
...
dat_2224 <- read.csv(paste0(path_data, "canum_old_format_22-24.csv") ,sep = ";")
...
ggplot(data = split, aes(x = year, y = wbss)) +
  geom_line() +
  facet_grid(wr~area+quarter)
